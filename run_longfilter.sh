...
mkdir -p /mnt/blob-pretraining-hptraining/haoran_result/HELMET
...
for task in rag_short icl_short cite_short; do
  ...
  mkdir -p /mnt/blob-pretraining-hptraining/haoran_result/HELMET/${task}/${model_name}_${checkpoint_name}
  # Move the contents of the output directory to the target location
  mv output/${task}/${model_name}_${checkpoint_name}/* /mnt/blob-pretraining-hptraining/haoran_result/HELMET/${task}/${model_name}_${checkpoint_name}/
  # Remove the empty local output directory
  rmdir output/${task}/${model_name}_${checkpoint_name}
  echo "Task ${task} results moved to /mnt/blob-pretraining-hptraining/haoran_result/HELMET/${task}/${model_name}_${checkpoint_name}"
done
